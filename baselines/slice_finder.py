...
from baselines.risk_control import *
...
def plot_slices(data: pd.DataFrame,
                x_column: str,
                y_column: str,
                target: Iterable,
                slices: list,
                ...
                ax=None):
    """Plot a 2D scatterplot showing the samples, its classes, and the slices passed as parameters
    to the function"""
    if ax is None:
        ax = plt.gca()
    ...
    for s in slices:
        ...
        rules = s.filters
        if len(rules) < 2:
            rule = [k for k in rules.keys()][0]
            if isinstance(rules, dict):
                rule1_upperbound = rules[rule][0][1]
                rule1_lowerbound = rules[rule][0][0]
                rule1_attribute = rule
            ...
            rule1, rule2 = tuple(rules.keys())
            if isinstance(rules, dict):
                rule1_upperbound = rules[rule1][0][1]
                rule1_lowerbound = rules[rule1][0][0]
                rule1_attribute = rule1
                if rule1_lowerbound == float("-inf"):
                    rule1_lowerbound = data[rule1_attribute].min()
                if rule1_upperbound == float("inf"):
                    rule1_upperbound = data[rule1_attribute].max()
            ...
            if isinstance(rules, dict):
                rule2_upperbound = rules[rule2][0][1]
                rule2_lowerbound = rules[rule2][0][0]
                rule2_attribute = rule2
        ...
        if rule1_attribute == x_column:
            ...
            if plot_mean_error:
                ax.text(rule1_lowerbound, rule2_lowerbound, round(s.metric, 4), fontsize=8)
            ...
            ax.add_patch(plt.Rectangle((rule2_lowerbound - delta, rule1_lowerbound - delta),
                                       ...
                                       fill=False, edgecolor=color, linewidth=1))
            if plot_mean_error:
                ax.text(rule2_lowerbound, rule1_lowerbound, round(s.metric, 4), fontsize=8)
            # ax.text(rule2_upperbound, rule1_upperbound, round(subgroup.mean_dataset, 4), fontsize=8)
    return ax

def run_sf_baseline(model, X_sd, y, k):
    """Runs the complete framework and returns the slices found and the coverage of each slice"""
    sf = SliceFinder(model, (X_sd.drop(columns=['errors']), pd.Series(y)))
    slices = sf.find_slice(k=k, epsilon=0.6, alpha=0.05, degree=2, risk_control=True, max_workers=4)
    # Calculate coverage used afterwards
    slice_cov = [X_sd.index.isin(x.data_idx) for x in slices]
    return slices, slice_cov
